...
public class Graph {
    ...
    public Noeud getNoeudaleatoire() {
        double res = Math.random() * this.getHmap().keySet().size();
        Noeud n = null;
        for (int i : this.getHmap().keySet()) {
            n = this.getHmap().get(i);
            if (!(n.getId() == (int) res))
                continue;
            else
                break;
    ...
    public void addNoeud(int n) {
        ...
        this.getNoeuds().add(new Noeud(n));
    ...
    public void GraphNonOriente(String file) {
        try (Reader reader = Files.newBufferedReader(Paths.get(file));
             CSVParser csvParser = new CSVParser(reader, CSVFormat.DEFAULT);) {
            for (CSVRecord csvRecord : csvParser) {
                // Accessing Values by Column Index
                String res = csvRecord.get(0);
                int src = Integer.parseInt(csvRecord.get(0));
                int cbl = Integer.parseInt(csvRecord.get(1));
                double poid = Double.parseDouble(csvRecord.get(2));
                ...
                this.addArcNonOriente(src, cbl, poid);
            }
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
    ...
    public void addArcNonOriente(int x, int y, double poids) {
        if (this.getNoeud(x) != null && this.getNoeud(y) != null) {
            if (!this.getNoeud(x).hasSuccesseur(y) && !this.getNoeud(y).hasSuccesseur(x)) {
                Arc a1 = new Arc(this.getNoeud(x), this.getNoeud(y), poids);
                Arc a2 = new Arc(this.getNoeud(y), this.getNoeud(x), poids);
                this.getNoeud(x).getSucc().add(a1);
                this.getNoeud(y).getSucc().add(a2);
            }
        }
    }
}